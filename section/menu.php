<section class="menu_area">
    <!-- Navbar -->
    <nav class="navbar navbar-expand-lg navbar-light">
        <div class="container">
            <a class="navbar-brand fw-bold text-dark" href="index.php">
                <img src="inclode/img/logo.png" alt="Logo" height="50" class="me-2">
            </a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item"><a class="nav-link active" href="index.php">Home</a></li>
                    <li class="nav-item"><a class="nav-link" href="about.php">About Us</a></li>


                    <li class="nav-item">
                        <div class="dropdown">
                            <a class="nav-link" data-bs-toggle="dropdown" aria-expanded="false">
                                Corporate Profile<i style="font-size: 15px;" class="fas fa-chevron-down"></i>
                            </a>
                            <ul class="dropdown-menu">

                                <li><a class="dropdown-item" href="profile.php">Board of Directors</a></li>
                                <li><a class="dropdown-item" href="#">Group Companies</a></li>
                                <li><a class="dropdown-item" href="#">Departments</a></li>

                            </ul>
                        </div>
                    </li>

                    <li class="nav-item">
                        <div class="dropdown">
                            <a class="nav-link" data-bs-toggle="dropdown" aria-expanded="false">
                                Program<i style="font-size: 15px;" class="fas fa-chevron-down"></i>
                            </a>
                            <ul class="dropdown-menu">

                                <li><a class="dropdown-item" href="program.php">Program</a></li>
                                <li><a class="dropdown-item" href="#">GENKI</a></li>
                                <li><a class="dropdown-item" href="#">Sesame</a></li>
                                <li><a class="dropdown-item" href="#">Mung Dal</a></li>
                                <li><a class="dropdown-item" href="#">Wellness</a></li>
                                <li><a class="dropdown-item" href="#">Social Procurement</a></li>
                            </ul>
                        </div>
                    </li>


                    <li class="nav-item"><a class="nav-link" href="blog.php">Blog</a></li>
                    <li class="nav-item">
                        <div class="dropdown">
                            <a class="nav-link" data-bs-toggle="dropdown" aria-expanded="false">
                                Gallery <i style="font-size: 15px;" class="fas fa-chevron-down"></i>
                            </a>
                            <ul class="dropdown-menu">
                                <li><a class="dropdown-item" href="gallery.php">Photo Gallery</a></li>
                                <li><a class="dropdown-item" href="video.php">Video Gallery</a></li>
                            </ul>
                        </div>
                    </li>
                    <li class="nav-item d-block d-lg-none"><a class="nav-link" href="#">what is Euglena?</a></li>
                    <li class="nav-item d-block d-lg-none"><a class="nav-link" href="#">What is Chlotrlla?</a></li>


                </ul>
            </div>
        </div>
    </nav>
</section>